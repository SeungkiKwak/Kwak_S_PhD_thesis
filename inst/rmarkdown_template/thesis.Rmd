...
```{r, cache=FALSE, child="chapter2.Rmd"}
```

```{r, cache=FALSE, child="chapter3.Rmd"}
# add more chunks like this to include additional chapters in the PDF
```
...
git_data <- repmis::git_stamp(repo = getwd())
```

The current git commit of this file is `r git_data[1]`, which is on the `r git_data[2]` branch. The current commit message is "`r git_data[3]`". 
